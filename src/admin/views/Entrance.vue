  ...
  <div class="common-main">
    ...
    <div class="common-content">
      <el-table :data="tableData" v-loading="tableLoading" height="100%">
        ...
        <el-table-column label="企微拒绝" width="80px" align="center" class-name="joinable_wc">
          <template #default="scope">
            <el-dropdown @command="onJoinablewcCommand($event, scope.row)">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item :command="0" :disabled="scope.row.joinable_wc === 0">待验</el-dropdown-item>
                  <el-dropdown-item :command="1" :disabled="scope.row.joinable_wc === 1">正常</el-dropdown-item>
                  <el-dropdown-item :command="2" :disabled="scope.row.joinable_wc === 2">失效</el-dropdown-item>
                  <el-dropdown-item :command="3" :disabled="scope.row.joinable_wc === 3">异常</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </template>
...
<script setup>
  ...
  // 更改状态
  function onJoinablewcCommand (joinable_wc, row) {
    axios.post('admin/entrance/itemJoinablewc', { eid: row.id, joinable_wc: joinable_wc }).then(() => {
      ElMessage.success('操作成功')
      row.joinable_wc = joinable_wc
    }).catch(() => { })
  }

<template>
            ...
            <div class="entrance">
              <el-table :ref="'entrance_table_' + props.row.id" :show-header="false" :data="props.row.entrance"
                ...
                <el-table-column label="总人数">
                  <template #default="scope">成员：{{scope.row.members=== null ? '未知' : scope.row.members}}</template>
                </el-table-column>
                ...
                <el-table-column label="有效期">
                  <template #default="scope">到期时间：{{scope.row.expire_date}}</template>
                </el-table-column>
                ...
                <el-table-column label="企微拒绝" prop="status">
                  <template #default="scope">
                    <!-- <span style="opacity:0.7;"
                      :style="{color: scope.row.joinable === 1 ? 'var(--el-color-success)'  : 'var(--el-color-danger)'}">{{scope.row.joinable
                      === 1 ? '可加群' : '不可加群'}}</span> -->
                    <span
  ...
  <!-- 添加活码对话框 -->
  <el-dialog v-model="showAddDlg" title="提交活码" :width="350" @closed="onAddDlgClosed" @open="onAddDlgOpen"
    ...
    <el-form label-width="50px" :model="formData" :rules="formRules" ref="addFormRef">
      ...
      <el-form-item label="限购" prop="price">
...
<script setup>
  ...
  const loadData = () => {
    ...
    axios.post('provider/qrcode/index', filter, {
      params: {
